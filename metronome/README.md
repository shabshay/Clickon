...
The workflow in `.github/workflows/gh-pages.yml` builds the app on pushes to `main` and publishes the `dist` folder to GitHub Pages. To turn it on in your fork:
...
3. Push to `main` (or manually run the workflow). The action will upload the build and create the `github-pages` deployment.
